"""
Together AI provider — fast inference, free tier available.
Get free API key at: https://api.together.xyz
Free models: Llama-3.3-70B-Instruct-Turbo-Free, Llama-Vision-Free
"""
...
class TogetherProvider(BaseProvider):
    ...
    def __init__(self, config):
        ...
        if not config.together_api_key:
            raise ProviderError(
                "TOGETHER_API_KEY not set. Get a free key at: https://api.together.xyz"
            )
        self.api_key = config.together_api_key
        self.model = config.model or "meta-llama/Llama-3.3-70B-Instruct-Turbo-Free"
    ...
    def _request(self, payload: dict) -> dict:
        ...
        try:
            ...
            raise ProviderError(f"Together API error {e.code}: {body[:500]}")
        except Exception as e:
            raise ProviderError(f"Together request failed: {e}")
